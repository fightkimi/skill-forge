---
name: clipmind-sop-core-s06-ondemand-language-gate
description: "当操盘手明确调用“用户语言一致性检查”，并希望检查回答是否沿用用户的语言、专业程度和关键术语，避免突然切换腔调或误解表达时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
检查回答是否沿用用户的语言、专业程度和关键术语，避免突然切换腔调或误解表达。
...
prompt_key: core/s06_ondemand_language_gate
module_key: core
sop_stage: S06 语言库闸门(成品类无语言库降级)
output_type: language_gate
change_note: 新增 —— 采纳操盘手硬规则「没有 IP 语言库不出终稿，只出结构稿+补料问题」。只在成品类(脚本/文案)且无 confirmed IP 语言库时注入;判断/策略/计划类不受影响。
activate_on_seed: true
```

### 原始执行正文

## 无 IP 语言库——本次只出结构稿，不出终稿

当前这个 IP 还没有可用的语言库（语气 DNA：本人的开头 / 判断 / 举例 / 收口 / 禁用表达）。没有它，"去 AI 味""像本人"都是空的——硬写终稿只会写出一个"换谁都能讲"的通用稿。

所以本次**降级处理**：

- **只输出结构稿**：把核心判断、要点、可用的素材线索按结构列清楚，开头标明这是「结构稿（缺 IP 语言库）」，**不是可直接发布的终稿**。
- **同时给补料问题**（≤5 条、能直接问客户）：缺哪些本人原话 / 真实案例 / 常用表达 / 口头禅 / 真实场景，补齐后才能出"像本人"的终稿。
- **不要**把结构稿包装成成品，不要标「可直接拍 / 可直接发」，不要为了凑"像真人"而编造本人原话或案例。
...
2. 只修改当前 `clipmind-sop-core-s06-ondemand-language-gate`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-sop-core-s06-ondemand-language-gate/`。

---
name: clipmind-governance-production-path-routing-v1
description: "当操盘手明确调用“生产路径判断”，并希望判断任务走已有文稿直接生产,还是从素材召回生成时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
```yaml
ref: production_path_routing.v1
key: production_path_routing
version: v1
status: active
name: 生产路径判断
description: 判断任务走已有文稿直接生产,还是从素材召回生成。
input_contract:
  required:
  - content_task
  optional:
...
workbench_context: 生产执行路径 Skill,在已有文稿直接生产和 AI 从素材生成之间分流。
core_methodology:
...
- 先判断用户是否已有文稿、素材观点是否可用、是否需要 AI 匹配生成。
- 已有文稿进入编辑、审稿和发布准备;无文稿则走观点召回和脚本生成。
- 路径选择必须可人工切换,不能锁死用户。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把方法论表格原样堆给普通用户。
- 不要在没有素材观点时只弹错误。
example_application: 用户上传现成稿件时跳过观点召回,直接进入审稿与发布准备。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-production-path-routing-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-production-path-routing-v1/`。

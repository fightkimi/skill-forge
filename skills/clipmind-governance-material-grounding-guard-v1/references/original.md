---
name: clipmind-governance-material-grounding-guard-v1
description: "当操盘手明确调用“素材贴合与反幻觉”，并希望检查输出是否真正来自上传资料,阻止通用经验冒充资料结论时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
```yaml
ref: material_grounding_guard.v1
key: material_grounding_guard
version: v1
status: active
name: 素材贴合与反幻觉
description: 检查输出是否真正来自上传资料,阻止通用经验冒充资料结论。
input_contract:
  required:
  - candidate_output
  - source_materials
...
- agent_studio
missing_input_rules:
- 缺少 source_materials 时标记 partial_with_missing_inputs。
surface_constraints:
  display_text: 40
  detail_text: true
...
workbench_context: 资料接地 Skill,负责确认模型是否真正消费了本轮资料,防止 baseline 伪成功。
core_methodology:
...
- 检查输入快照中的资料是否进入模型可读上下文。
- 识别「未获取有效资料」等模型空答,并阻止与本地兜底合并成成功。
- 要求输出里至少出现资料证据、来源或明确缺口。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 没有资料证据时不能标记为 AI 建档成功。
anti_patterns:
- 不要用预设行业话术替代当前资料。
- 不要把没有来源的判断写成事实。
- 不要把方法论表格原样堆给普通用户。
- 不要让本地关键词 baseline 伪装成模型理解。
example_application: AI 回答没拿到资料时,把建档结果标记为可重试而不是生成泛化草稿。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-material-grounding-guard-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-material-grounding-guard-v1/`。

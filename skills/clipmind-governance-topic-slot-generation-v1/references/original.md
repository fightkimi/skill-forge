---
name: clipmind-governance-topic-slot-generation-v1
description: "当操盘手明确调用“选题生成与槽位拆解”，并希望把阶段目标拆成对应数量的选题、槽位和发布节奏时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
```yaml
ref: topic_slot_generation.v1
key: topic_slot_generation
version: v1
status: active
name: 选题生成与槽位拆解
description: 把阶段目标拆成对应数量的选题、槽位和发布节奏。
input_contract:
  required:
  - stage_plan
  - target_count
...
workbench_context: 内容计划选题 Skill,把阶段目标转成内容槽位和题目建议。
core_methodology:
...
- 根据五类内容选择每条内容承担痛点、认知、故事、证明或成交任务。
- 每个选题必须绑定阶段主线、情绪维度、用户痛点和生产承接。
- 只在详情区保留完整选题线索,Week 卡主视图只放短摘要。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把方法论表格原样堆给普通用户。
- 不要输出通用四周主题或多行标签墙。
example_application: 围绕中小老板增长焦虑生成一条证明内容和一条成交内容。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-topic-slot-generation-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-topic-slot-generation-v1/`。

---
name: clipmind-governance-attribution-evidence-v1
description: "当操盘手明确调用“归因证据判断”，并希望区分真实数据、人工判断和推测,避免误沉淀方法时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
```yaml
ref: attribution_evidence.v1
key: attribution_evidence
version: v1
status: active
name: 归因证据判断
description: 区分真实数据、人工判断和推测,避免误沉淀方法。
input_contract:
  required:
  - content_result
  - monitor_snapshots
...
workbench_context: 归因证据 Skill,负责把复盘结论连接到内容、发布、监控和方法资产证据。
core_methodology:
...
- 不要把方法论表格原样堆给普通用户。
- 不要用单一指标解释全部结果。
example_application: 把低咨询归因到 CTA 不清晰时,引用发布文案和评论反馈。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-attribution-evidence-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-attribution-evidence-v1/`。

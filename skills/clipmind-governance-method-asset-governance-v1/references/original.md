---
name: clipmind-governance-method-asset-governance-v1
description: "当操盘手明确调用“方法资产治理”，并希望判断方法是否可沉淀、适用哪些 IP/平台和是否要迭代时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
```yaml
ref: method_asset_governance.v1
key: method_asset_governance
version: v1
status: active
name: 方法资产治理
description: 判断方法是否可沉淀、适用哪些 IP/平台和是否要迭代。
input_contract:
  required:
  - method_candidate
  optional:
...
workbench_context: 方法资产治理 Skill,判断爆款方法、复盘经验和 Agent 输出是否能沉淀为资产。
core_methodology:
...
- 评估适配性、证据强度、样本覆盖数、影响环节和风险。
- 区分建议改 Prompt、作为方法资产引用和暂不处理。
- 通过评估后只能生成候选或草稿,不自动激活。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把方法论表格原样堆给普通用户。
- 不要把建议型方法改成自动决策规则。
example_application: 三条高表现内容都验证同一钩子结构后,沉淀为生产可引用方法。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-method-asset-governance-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-method-asset-governance-v1/`。

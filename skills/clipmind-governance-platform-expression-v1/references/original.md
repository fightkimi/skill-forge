---
name: clipmind-governance-platform-expression-v1
description: "当操盘手明确调用“平台表达适配”，并希望根据平台调整内容节奏、长度、标题、语气、CTA 和发布前检查点时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
根据平台调整内容节奏、长度、标题、语气、CTA 和发布前检查点。
...
```yaml
ref: platform_expression.v1
key: platform_expression
version: v1
status: active
name: 平台表达适配
description: 根据平台调整内容节奏、长度、标题、语气、CTA 和发布前检查点。
input_contract:
  required:
  - platform
  - content_goal
...
workbench_context: 平台表达 Skill,把同一策略翻译成小红书、抖音、视频号、B 站等平台适配表达。
core_methodology:
...
- 先保留 IP 的关系角色和买点,再改写平台表达。
- 按平台识别开场、标题、正文、话题、互动和行动入口。
- 明确哪些内容是平台表达建议,哪些是客户/IP 长期定位。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把方法论表格原样堆给普通用户。
- 不要只套平台热词而丢掉当前 IP 证据。
example_application: 把同一条创业陪跑观点翻译成小红书图文标题和抖音口播开场。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-platform-expression-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-platform-expression-v1/`。

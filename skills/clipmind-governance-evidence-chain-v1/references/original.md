---
name: clipmind-governance-evidence-chain-v1
description: "当操盘手明确调用“素材证据链判断”，并希望判断观点、素材、事实、证据缺口和来源可靠性,避免把假设包装成事实时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
判断观点、素材、事实、证据缺口和来源可靠性,避免把假设包装成事实。
...
```yaml
ref: evidence_chain.v1
key: evidence_chain
version: v1
status: active
name: 素材证据链判断
description: 判断观点、素材、事实、证据缺口和来源可靠性,避免把假设包装成事实。
input_contract:
  required:
  - business_context
  - source_materials
...
workbench_context: 所有模块的证据链 Skill,负责判断结论是否来自真实资料、档案、策略、任务或监控信号。
core_methodology:
...
- 把每个判断拆成来源、摘录、适用边界、不确定性四段。
- 对无来源的数字、行业变化、案例和效果承诺降级为待核实依据。
- 用证据强弱区分可确认结论、仅参考线索和暂不采用内容。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把方法论表格原样堆给普通用户。
- 不要用「有资料依据」空标签替代证据。
example_application: 建档时把「用户痛点」指向会议纪要里的原话,并说明这句话只支持痛点判断,不直接支持成交承诺。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-evidence-chain-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-evidence-chain-v1/`。

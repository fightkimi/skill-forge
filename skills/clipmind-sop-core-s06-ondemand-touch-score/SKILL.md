---
name: clipmind-sop-core-s06-ondemand-touch-score
description: "当操盘手明确调用“用户触达评分规范”，并希望从用户共鸣、信任推进、价值感知和行动触发四个方向评估内容的真实触达能力时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
从用户共鸣、信任推进、价值感知和行动触发四个方向评估内容的真实触达能力。
...
```yaml
skill: S06
prompt_key: core/s06_ondemand_touch_score
module_key: core
output_type: ondemand_fragment
trigger: 涉及场景 / 痛点 / 标题 / 脚本 / 朋友圈 / 短视频开头
change_note: 按需片段 —— doc00 §6B 触感评分表
```

### 原始执行正文

# 按需 · 触感评分(涉及场景 / 标题 / 脚本 / 朋友圈时追加)

凡涉及场景、痛点、标题、脚本、朋友圈、短视频开头,先做触感评分。6 分满分,低于 4 分不进生产执行。

| 项目 | 合格标准 |
|---|---|
| 具体人物 | 能看出是谁和谁,不是「用户 / 客户」这种泛称 |
| 具体现场 | 有时间、地点、动作、物品或对话 |
| 崩溃瞬间 | 有尴尬、后悔、损失、失控或不敢说出口的瞬间 |
| 用户原话 | 有一句用户可能真实说出口的话 |
| 行动后果 | 说清这件事之后关系、信任、购买或行动怎么变 |
| 能被转述 | 用户看完能用一句人话复述给朋友 |

场景不是标签。只写「旅行吵架 / 客户流失 / 成交困难」不合格,必须写到用户能看到画面、想起自己、产生「我是不是也该操心一下」的反应。
...
2. 只修改当前 `clipmind-sop-core-s06-ondemand-touch-score`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-sop-core-s06-ondemand-touch-score/`。

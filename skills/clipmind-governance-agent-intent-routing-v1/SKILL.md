---
name: clipmind-governance-agent-intent-routing-v1
description: "当操盘手明确调用“Agent 意图路由”，并希望根据用户问题选择合适 Agent 或工作流分类,并识别是否需要收束需求时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
根据用户问题选择合适 Agent 或工作流分类,并识别是否需要收束需求。
...
```yaml
ref: agent_intent_routing.v1
key: agent_intent_routing
version: v1
status: active
name: Agent 意图路由
description: 根据用户问题选择合适 Agent 或工作流分类,并识别是否需要收束需求。
input_contract:
  required:
  - user_message
  optional:
...
workbench_context: Agent 工坊意图路由 Skill,把用户自由任务落回当前项目流程和合适 Agent。
core_methodology:
...
- 默认结合当前客户/IP 档案、策略、素材、计划和生产状态。
- 按档案理解、策略判断、内容计划、生产执行、发布准备、监控复盘、通用创作分类推荐 Agent。
- 用户明确要求通用方法时才脱离当前 IP。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把没有来源的判断写成事实。
- 不要把方法论表格原样堆给普通用户。
- 不要把内部 Skill、Prompt 或工具说明塞进对话框。
example_application: 用户问怎么做小红书,默认回答当前 IP 的小红书内容任务而不是泛泛教学。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-agent-intent-routing-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-agent-intent-routing-v1/`。

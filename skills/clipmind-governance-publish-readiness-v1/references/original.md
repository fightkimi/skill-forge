---
name: clipmind-governance-publish-readiness-v1
description: "当操盘手明确调用“发布准备检查”，并希望检查平台、版本、时间、素材授权和风险确认是否齐备时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
```yaml
ref: publish_readiness.v1
key: publish_readiness
version: v1
status: active
name: 发布准备检查
description: 检查平台、版本、时间、素材授权和风险确认是否齐备。
input_contract:
  required:
  - content_task
  - platform
...
workbench_context: 发布准备 Skill,判断内容是否具备平台、版本、时间、文案和承接路径。
core_methodology:
...
- 检查平台、版本、发布时间、发布文案、承接路径和回填节点。
- 区分发布准备队列、今日发布内容和月历事实记录。
- 缺字段时给可操作保存入口,不能重复内容计划语义。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 发布排期只管最终发布事实,不重新规划内容。
anti_patterns:
- 不要用预设行业话术替代当前资料。
- 不要把没有来源的判断写成事实。
- 不要把方法论表格原样堆给普通用户。
- 不要在发布页展示内容计划的大段策略。
example_application: 待发布内容设置平台和 5月18日 20:00 后进入月历。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-publish-readiness-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-publish-readiness-v1/`。

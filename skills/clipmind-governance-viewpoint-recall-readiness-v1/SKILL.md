---
name: clipmind-governance-viewpoint-recall-readiness-v1
description: "当操盘手明确调用“观点召回与可用性判断”，并希望判断素材观点是否足够进入生产下一步,并提供生成/重试观点入口语义时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
判断素材观点是否足够进入生产下一步,并提供生成/重试观点入口语义。
...
```yaml
ref: viewpoint_recall_readiness.v1
key: viewpoint_recall_readiness
version: v1
status: active
name: 观点召回与可用性判断
description: 判断素材观点是否足够进入生产下一步,并提供生成/重试观点入口语义。
input_contract:
  required:
  - materials
  - content_task
...
workbench_context: 观点召回准备 Skill,判断生产前是否已有足够素材观点可召回。
core_methodology:
...
- 检查素材是否已处理、观点是否已提炼、当前任务是否有可匹配观点。
- 缺观点时提供上传素材、查看处理状态、生成/重试素材观点入口。
- 观点不足时只能给补齐引导,不能推进脚本生成。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把方法论表格原样堆给普通用户。
- 不要用技术错误替代业务引导。
example_application: 生产第一步提示「还缺素材观点,先生成素材观点」并定位到素材库。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-viewpoint-recall-readiness-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-viewpoint-recall-readiness-v1/`。

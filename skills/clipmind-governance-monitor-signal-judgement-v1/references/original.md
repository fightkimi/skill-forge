---
name: clipmind-governance-monitor-signal-judgement-v1
description: "当操盘手明确调用“监控信号判断”，并希望基于 2h/24h/72h 数据判断异常、机会和调整方向时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
```yaml
ref: monitor_signal_judgement.v1
key: monitor_signal_judgement
version: v1
status: active
name: 监控信号判断
description: 基于 2h/24h/72h 数据判断异常、机会和调整方向。
input_contract:
  required:
  - monitor_snapshots
  optional:
...
- 不得把推测写成归因结论。
sample_refs:
- monitor_signal_analysis_v1
- retrospect_summary_learning_v1
applicable_modules:
- feedback
missing_input_rules:
- 缺少监控数据时返回 missing_inputs=monitoring。
surface_constraints:
  display_text: 40
  detail_text: true
...
workbench_context: 投放监控 Skill,判断 2h/24h/72h 信号和需要补录的数据。
core_methodology:
...
- 按 2h 初筛、24h 追投、72h 复盘拆解信号。
- 无数据时显示待补数据,不能说暂无异常。
- 点击检查点必须定位到对应队列和数据录入动作。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把方法论表格原样堆给普通用户。
- 不要把空数据当低风险。
example_application: 72h 没回填时卡片点击后聚焦到待补数据项。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-monitor-signal-judgement-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-monitor-signal-judgement-v1/`。

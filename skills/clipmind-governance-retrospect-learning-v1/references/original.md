---
name: clipmind-governance-retrospect-learning-v1
description: "当操盘手明确调用“复盘学习与方法沉淀”，并希望从表现数据、人工判断和业务结果中形成下周期调整与方法库候选时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
```yaml
ref: retrospect_learning.v1
key: retrospect_learning
version: v1
status: active
name: 复盘学习与方法沉淀
description: 从表现数据、人工判断和业务结果中形成下周期调整与方法库候选。
input_contract:
  required:
  - content_result
  - monitor_snapshots
...
- 不得伪造表现数据。
sample_refs:
- retrospect_summary_learning_v1
- method_writeback_candidate_v1
applicable_modules:
- feedback
- retrospect
- method_library
...
methodology_anchors:
- retrospect_attribution
- method_asset_governance
- evidence_grounding
```
...
workbench_context: 复盘学习 Skill,把发布和监控数据转成可复用的策略、内容和方法资产候选。
core_methodology:
...
- 按定位检验、痛点命中、信任推进、行动触发、内容校准五项复盘。
- 区分高表现原因、低表现原因、证据不足和下一轮验证假设。
- 只把多条证据支持的方法沉淀为资产候选。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 复盘结论不能只看播放或点赞,必须看信任和行动信号。
anti_patterns:
- 不要用预设行业话术替代当前资料。
- 不要把没有来源的判断写成事实。
- 不要把方法论表格原样堆给普通用户。
- 不要用单条偶然数据改长期策略。
example_application: 72h 复盘发现评论多但咨询少,回到买点设计和承接链路调整。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-retrospect-learning-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-retrospect-learning-v1/`。

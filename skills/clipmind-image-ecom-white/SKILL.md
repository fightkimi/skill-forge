---
name: clipmind-image-ecom-white
description: "当操盘手明确调用“产品白底图”，并希望电商主图可上架时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
```text
电商产品白底图,产品居中,纯白背景,影棚级布光,锐利细节,可直接上架。产品:{subject}。方图。
```
...
2. 只修改当前 `clipmind-image-ecom-white`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-image-ecom-white/`。

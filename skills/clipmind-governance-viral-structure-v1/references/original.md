---
name: clipmind-governance-viral-structure-v1
description: "当操盘手明确调用“爆款结构拆解”，并希望沉淀标题、开头、情绪推进、结构转折、证据摆放和转化收口的可复用方法时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
沉淀标题、开头、情绪推进、结构转折、证据摆放和转化收口的可复用方法。
...
```yaml
ref: viral_structure.v1
key: viral_structure
version: v1
status: active
name: 爆款结构拆解
description: 沉淀标题、开头、情绪推进、结构转折、证据摆放和转化收口的可复用方法。
input_contract:
  required:
  - content_sample
  - performance_signal
...
- 不得复制高风险或不可核验表达。
sample_refs:
- method_extract_viral_structure_v1
...
methodology_anchors:
- viral_structure_decomposition
- reuse_condition_check
- method_asset_boundary
```
...
workbench_context: 爆款结构 Skill,负责提炼钩子、冲突、转折、证明和行动承接,但不牺牲真实依据。
core_methodology:
...
- 把爆款拆成痛点命名、认知反差、真实场景、证据证明、行动入口。
- 判断结构是否适合当前 IP 的关系角色和内容禁忌。
- 只把可迁移结构沉淀为方法,不复制具体话题和未经验证结论。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把没有来源的判断写成事实。
- 不要把方法论表格原样堆给普通用户。
- 不要把偶然爆款直接污染长期 Prompt 或正式策略。
example_application: 把高互动创业案例拆成「误区-反转-路径-证明」结构,再用于当前 IP 的内容计划。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-viral-structure-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-viral-structure-v1/`。

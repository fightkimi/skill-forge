---
name: clipmind-governance-content-generation-reference-v1
description: "当操盘手明确调用“内容生成轻量参考”，并希望将场景、痛点层级、表达结构、变量填充和边界检查作为内容生成类 Prompt/Agent 的内部参考,提升草案具体度而不新增用户模块时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
将场景、痛点层级、表达结构、变量填充和边界检查作为内容生成类 Prompt/Agent 的内部参考,提升草案具体度而不新增用户模块。
...
```yaml
ref: content_generation_reference.v1
key: content_generation_reference
version: v1
status: active
name: 内容生成轻量参考
description: 将场景、痛点层级、表达结构、变量填充和边界检查作为内容生成类 Prompt/Agent 的内部参考,提升草案具体度而不新增用户模块。
input_contract:
  required:
  - generation_task
  - ip_context
...
quality_rules:
- 只用于图文、脚本、标题、改写、平台变体或单条内容草案等生成类场景。
- 输出必须更具体地落到用户场景、痛点延续、表达结构和下一步行动。
- 信息不足时必须写成待补资料或人工确认点,不能编造案例、数据、来源或业务结果。
guardrails:
- 不得新增内容生成引擎页面、Pipeline Core 或 DB schema。
- 不得把变量清单、自检表或内部步骤原样展示给普通用户。
- 不得影响事实核查、证据链、行业研究、投放监控、复盘归因等非生成场景。
sample_refs:
- content_plan_source_trace_v1
- script_hook_generation_v1
applicable_modules:
- production
- agent_studio
missing_input_rules:
...
workbench_context: 内容生成 Skill,把内容生成引擎作为轻量参考,服务脚本、图文、选题和口播草稿。
core_methodology:
...
- 先确定本条内容属于五类内容中的痛点内容、认知内容、故事内容、证明内容或成交内容。
- 按内容类型组织结构,例如痛点内容用具体场景、问题命名、原因拆解、判断给出、路径暗示。
- 把变量清单和自检表留在内部,用户只看到可编辑成稿和确认点。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 成稿必须有当前 IP 的事实、语气和行动入口。
anti_patterns:
- 不要用预设行业话术替代当前资料。
- 不要把没有来源的判断写成事实。
- 不要把方法论表格原样堆给普通用户。
- 不要输出空泛创作模板或变量清单。
example_application: 生产执行里把会议纪要中的真实转折写成一条小红书图文草稿。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-content-generation-reference-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-content-generation-reference-v1/`。

---
name: clipmind-governance-viewpoint-material-tagging-v1
description: "当操盘手明确调用“观点提炼与素材标签”，并希望从资料中提炼观点、事实、案例、风险和适用场景,供生产召回使用时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
从资料中提炼观点、事实、案例、风险和适用场景,供生产召回使用。
...
```yaml
ref: viewpoint_material_tagging.v1
key: viewpoint_material_tagging
version: v1
status: active
name: 观点提炼与素材标签
description: 从资料中提炼观点、事实、案例、风险和适用场景,供生产召回使用。
input_contract:
  required:
  - materials
  optional:
...
workbench_context: 素材观点 Skill,把素材正文转成观点、痛点、场景、证据和内容元素标签。
core_methodology:
...
- 不要把方法论表格原样堆给普通用户。
- 不要把标签墙默认堆到用户卡片上。
example_application: 把会议记录里的客户异议标成「信任不足/价格顾虑/低风险第一步」。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-viewpoint-material-tagging-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-viewpoint-material-tagging-v1/`。

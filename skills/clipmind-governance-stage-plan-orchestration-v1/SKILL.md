---
name: clipmind-governance-stage-plan-orchestration-v1
description: "当操盘手明确调用“阶段式内容计划编排”，并希望根据阶段数、日期、目标内容数和检查点生成阶段计划草稿时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
```yaml
ref: stage_plan_orchestration.v1
key: stage_plan_orchestration
version: v1
status: active
name: 阶段式内容计划编排
description: 根据阶段数、日期、目标内容数和检查点生成阶段计划草稿。
input_contract:
  required:
  - strategy
  - stage_count
...
- content_plan_platform_direction_v1
applicable_modules:
- content_plan
missing_input_rules:
- 缺少策略或日期时返回 missing_inputs。
surface_constraints:
  display_text: 24
  detail_text: true
...
workbench_context: 阶段式内容计划 Skill,把策略拆成可编辑阶段,再驱动内容建议数量和节奏。
core_methodology:
...
- 默认按 4 阶段组织,但允许人工调整为 3/5/6 个合理阶段。
- 每个阶段必须有目标内容数、平台重点、发布节奏、主线、情绪维度和检查点。
- 阶段草稿经人工确认后才生成下方内容建议。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 阶段写 5 条,下方不能自动生成 7 条。
anti_patterns:
- 不要用预设行业话术替代当前资料。
- 不要把没有来源的判断写成事实。
- 不要把方法论表格原样堆给普通用户。
- 不要把自然周 7 天当作默认内容数量。
example_application: 把一个月策略拆成认知破冰、信任证明、方法交付、转化验证四个阶段。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-stage-plan-orchestration-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-stage-plan-orchestration-v1/`。

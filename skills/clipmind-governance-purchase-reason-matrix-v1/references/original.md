---
name: clipmind-governance-purchase-reason-matrix-v1
description: "当操盘手明确调用“购买/信任理由矩阵”，并希望提炼用户为什么相信、为什么行动、为什么现在行动,并区分理性理由与情绪触发时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
提炼用户为什么相信、为什么行动、为什么现在行动,并区分理性理由与情绪触发。
...
```yaml
ref: purchase_reason_matrix.v1
key: purchase_reason_matrix
version: v1
status: active
name: 购买/信任理由矩阵
description: 提炼用户为什么相信、为什么行动、为什么现在行动,并区分理性理由与情绪触发。
input_contract:
  required:
  - target_user
  - pain_points
...
- 不得把建议直接写成最终转化承诺。
sample_refs:
- strategy_purchase_reason_v1
- launch_cta_review_v1
applicable_modules:
- strategy
missing_input_rules:
- 缺少上游资料时只能生成待人工复核草稿。
...
workbench_context: 策略、成交和转化 Skill,负责把资料里的价值翻译成用户为什么现在选择这个 IP。
core_methodology:
...
- 逐项填写被选择的理由、被感知的价值、被触发的行动。
- 用十八种付费价值识别情绪、身份、结果、确定性、效率、避坑、资源、认知、路径等买点。
- 把买点设计成目标、时间、身份、参与、反馈五个要素。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把方法论表格原样堆给普通用户。
- 不要把产品优点直接当购买理由。
example_application: 策略页把「短视频 3 秒钩子」转成中小老板愿意咨询的低风险诊断入口。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-purchase-reason-matrix-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-purchase-reason-matrix-v1/`。

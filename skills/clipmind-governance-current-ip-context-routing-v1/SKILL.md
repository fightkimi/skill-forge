---
name: clipmind-governance-current-ip-context-routing-v1
description: "当操盘手明确调用“当前 IP 上下文路由”，并希望默认结合当前客户/IP 上下文回答,同时识别泛问题、跨 IP 意图和权限边界时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
默认结合当前客户/IP 上下文回答,同时识别泛问题、跨 IP 意图和权限边界。
...
```yaml
ref: current_ip_context_routing.v1
key: current_ip_context_routing
version: v1
status: active
name: 当前 IP 上下文路由
description: 默认结合当前客户/IP 上下文回答,同时识别泛问题、跨 IP 意图和权限边界。
input_contract:
  required:
  - current_ip
  - user_message
...
workbench_context: 当前 IP 上下文 Skill,保证全局能力页默认进入上次或第一个有权限的客户/IP。
core_methodology:
...
- 优先使用上次使用且仍有权限的 IP,其次第一个有权限 IP。
- 无权限、归档或不存在时才进入选择/新建页。
- 涉及其它客户/IP 时必须做权限校验和用户意图判断。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把方法论表格原样堆给普通用户。
- 不要静默串用其它客户资料。
example_application: Agent 工坊打开时自动进入高总 IP,并提示默认结合当前 IP 回答。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-current-ip-context-routing-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-current-ip-context-routing-v1/`。

---
name: clipmind-governance-material-ingestion-readiness-v1
description: "当操盘手明确调用“资料读取与入库判断”，并希望判断资料是否可读、可入库、可用于建档或生产,并给出恢复动作时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
```yaml
ref: material_ingestion_readiness.v1
key: material_ingestion_readiness
version: v1
status: active
name: 资料读取与入库判断
description: 判断资料是否可读、可入库、可用于建档或生产,并给出恢复动作。
input_contract:
  required:
  - materials
  optional:
...
- profile
missing_input_rules:
- 没有任何资料时返回 missing_inputs=materials。
surface_constraints:
  display_text: 40
  detail_text: true
...
workbench_context: 资料处理 Skill,负责判断文件、录音、图片、链接和文字备注是否已可用于提炼。
core_methodology:
...
- 把资料状态翻译成已保存、正在读取、可提炼、处理超时可重试。
- 支持部分资料可用时继续批量提炼,失败资料进入本轮未纳入。
- 不暴露队列、worker 或技术任务名。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把方法论表格原样堆给普通用户。
- 不要让单条失败资料卡死整个资料池。
example_application: 6 条资料里 4 条已读取、2 条失败时,允许基于 4 条开始 AI 建档。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-material-ingestion-readiness-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-material-ingestion-readiness-v1/`。

---
name: clipmind-governance-strategy-synthesis-v1
description: "当操盘手明确调用“策略草案综合”，并希望把档案、资料、方法和用户目标整理成本轮策略草稿时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
```yaml
ref: strategy_synthesis.v1
key: strategy_synthesis
version: v1
status: active
name: 策略草案综合
description: 把档案、资料、方法和用户目标整理成本轮策略草稿。
input_contract:
  required:
  - ip_profile
  - materials
...
workbench_context: 策略确认 Skill,把档案、资料、行业变化和目标转成可人工确认的本轮策略。
core_methodology:
...
- 用优势×痛点×场景×关系×行动设计生成本轮策略主线。
- 把行业变化作为可选证据,无可信来源时只提示缺口,不阻断策略。
- 把长期内容主张作为只读依据,不重复维护为策略字段。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把方法论表格原样堆给普通用户。
- 不要把档案内容主张机械复制成策略主线。
example_application: 把创业陪跑 IP 的资料转成本轮「先补信任证据,再设计低风险咨询入口」策略。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-strategy-synthesis-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-strategy-synthesis-v1/`。

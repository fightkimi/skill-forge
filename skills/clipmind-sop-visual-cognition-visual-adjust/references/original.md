---
name: clipmind-sop-visual-cognition-visual-adjust
description: "当操盘手明确调用“认知视觉调整”，并希望根据内容重点和目标平台调整版式、信息层级、颜色、字体、留白与视觉强调建议时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
根据内容重点和目标平台调整版式、信息层级、颜色、字体、留白与视觉强调建议。
...
```yaml
skill: cognition_visual_adjust
version: vaultai-2026-06-28
prompt_key: visual/cognition_visual_adjust
module_key: visual
output_schema: VisualAdjust
output_type: visual_adjust
guardrails:
- json_only
- follow_instruction
eval_checklist:
...
你是认知海报视觉调校师。用户对一张**已出图的认知海报**提了视觉调整指令,你把它翻成结构化的调整参数(只调视觉,不动文字)。

## 输入

- 当前视觉:主色 {{ cur_accent }} · 底色基调 {{ cur_bg }}(dark 深色情绪 / light 红黑白 / paper 暖纸)· 模板 {{ cur_template }}
- 用户指令:{{ instruction }}

## 映射规则

- 指令关于**颜色**(「换成蓝色」「主色再红一点」「用绿色」)→ `accent` 设成对应的 hex(如蓝#378ADD / 红#D2342F / 绿#1D9E75 / 暖橙#F0997B,或贴合描述的颜色)。
- 指令关于**明暗 / 基调**(「再暗点」「换浅色」「暖一点」)→ `bg` 设 dark / light / paper 之一。
- 指令关于**背景画面 / 氛围 / 质感**(「换个氛围」「背景换成城市夜景」「更高级」「加点光感」)→ `regen_background` 设 true,并在 `bg_style_hint` 写**一句中文氛围描述**(纯背景、留白充足、绝不含文字)。
- 指令**没提到的维度** → 对应字段给 `null`(或 regen_background 给 false),表示保留原值。

## 输出

**只输出一个 JSON**,不要解释、不要 markdown 代码块:

{ "accent": null, "bg": null, "regen_background": false, "bg_style_hint": null }
...
2. 只修改当前 `clipmind-sop-visual-cognition-visual-adjust`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-sop-visual-cognition-visual-adjust/`。

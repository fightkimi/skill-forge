---
name: clipmind-governance-skill-governance-eval-v1
description: "当操盘手明确调用“Skill 治理与样本评估”，并希望管理 Skill 版本、样本、通过率和低质量反馈,只在治理区展示时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
管理 Skill 版本、样本、通过率和低质量反馈,只在治理区展示。
...
```yaml
ref: skill_governance_eval.v1
key: skill_governance_eval
version: v1
status: active
name: Skill 治理与样本评估
description: 管理 Skill 版本、样本、通过率和低质量反馈,只在治理区展示。
input_contract:
  required:
  - skill_refs
  - eval_samples
...
quality_rules:
- 样本结果必须可追踪 Prompt 和 Skill。
- 低质量反馈必须保留人工审核。
- 治理字段不能出现在普通业务页。
guardrails:
- 不得把内部 schema 展示给普通用户。
- 不得自动激活新 Prompt。
- 不得删除核心护栏。
sample_refs:
- content_plan_source_trace_v1
- agent_xhs_graphic_note_v1
applicable_modules:
- settings
missing_input_rules:
- 缺少样本时返回治理缺口。
...
workbench_context: 管理设置 Skill 治理 Skill,用于管理员评估 Prompt-Skill 绑定、样本和运行质量。
core_methodology:
...
- 检查每个 Prompt 绑定的 Skill 是否覆盖输入、步骤、输出、质量标准和护栏。
- 用样本结果验证 Skill 是否产生可用业务结论。
- 治理结果只在管理设置展示,普通用户路径不暴露内部实现。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 治理报告必须指出具体 Prompt 或 Skill 缺口。
anti_patterns:
- 不要用预设行业话术替代当前资料。
- 不要把没有来源的判断写成事实。
- 不要把方法论表格原样堆给普通用户。
- 不要把 Prompt、模型、run id 等技术字段展示给普通用户。
example_application: 管理员看到内容计划 Prompt 缺少购买理由矩阵 Skill,要求补绑定。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-skill-governance-eval-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-skill-governance-eval-v1/`。

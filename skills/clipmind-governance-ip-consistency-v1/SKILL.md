---
name: clipmind-governance-ip-consistency-v1
description: "当操盘手明确调用“IP 一致性判断”，并希望判断输出是否贴合当前身份定位、目标用户、用户痛点、内容主张和内容禁忌时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
判断输出是否贴合当前身份定位、目标用户、用户痛点、内容主张和内容禁忌。
...
```yaml
ref: ip_consistency.v1
key: ip_consistency
version: v1
status: active
name: IP 一致性判断
description: 判断输出是否贴合当前身份定位、目标用户、用户痛点、内容主张和内容禁忌。
input_contract:
  required:
  - ip_profile
  - candidate_output
...
quality_rules:
- 必须引用当前 IP 的核心档案字段判断一致性。
- 发现与内容禁忌冲突时必须标记风险。
- 不能把通用模板伪装成当前 IP 定制结论。
guardrails:
- 不得机械复制档案内容主张作为本轮策略。
- 不得忽略已确认内容禁忌。
- 不得自动覆盖人工策略。
...
methodology_anchors:
- ip_positioning_consistency
- audience_relationship_fit
...
workbench_context: 跨档案、策略、计划、生产和发布的一致性 Skill,保证同一客户/IP 的身份、对象和承诺不互相打架。
core_methodology:
...
- 用一句话定位、三类核心用户、五个高频痛点建立一致性基线。
- 检查当前输出是否偏离内容主张、内容禁忌和目标用户。
- 冲突时指出冲突字段,并建议回到档案或策略确认。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 同一 IP 不应在不同页面出现不同目标用户。
anti_patterns:
- 不要用预设行业话术替代当前资料。
- 不要把没有来源的判断写成事实。
- 不要把方法论表格原样堆给普通用户。
- 不要为了迎合单条内容而改写长期定位。
example_application: 内容计划生成前发现本轮选题面向宝妈,但档案目标用户是中小老板,提示先确认目标用户变化。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-ip-consistency-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-ip-consistency-v1/`。

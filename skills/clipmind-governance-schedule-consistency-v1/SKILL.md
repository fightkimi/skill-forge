---
name: clipmind-governance-schedule-consistency-v1
description: "当操盘手明确调用“发布排期一致性”，并希望保证内容计划、最终发布时间和发布排期显示一致时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
```yaml
ref: schedule_consistency.v1
key: schedule_consistency
version: v1
status: active
name: 发布排期一致性
description: 保证内容计划、最终发布时间和发布排期显示一致。
input_contract:
  required:
  - content_task
  - launch_records
...
workbench_context: 排期一致性 Skill,保证内容计划建议时间和发布排期事实记录不混淆。
core_methodology:
...
- 区分建议时间和最终发布时间。
- 最终发布时间确认后必须同步发布排期月历和今日发布内容。
- 改期时保护已发布、已确认发布时间和进入生产的内容。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 不要把方法论表格原样堆给普通用户。
- 不要把内容计划节奏当成已确认发布事实。
example_application: 内容计划里确认 5月18日 20:00 后,发布排期同日必须显示该内容。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-schedule-consistency-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-schedule-consistency-v1/`。

---
name: clipmind-sop-core-s06-ondemand-chat-format
description: "当操盘手明确调用“对话格式规范”，并希望把按需回答约束成清楚、直接、可继续沟通的对话结构，避免堆砌内部字段和任务面板语时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
把按需回答约束成清楚、直接、可继续沟通的对话结构，避免堆砌内部字段和任务面板语。
...
prompt_key: core/s06_ondemand_chat_format
module_key: core
sop_stage: S06 呈现格式(对话正文)
output_type: chat_format
change_note: 新增 —— 对话 / 非 JSON 正文的 Markdown 呈现格式(加粗小标题 / 列表分层 / 重点突出,单色黑不彩色)。只在非结构化产出场景注入,与 structured_output 互斥。
...
把给人阅读的正文用 Markdown 组织好,做到「条理清晰、层次分明、重点突出」:

- **分项小标题加粗**:每一块开头的小标题用 `**小标题**` 加粗(如 `**核心判断**`、`**直接可用版本**`、`**下一步**`),让人扫一眼就知道每段在讲什么。
- **并列要点用列表**:多条并列内容用 `-` 或 `1.` 逐条成行,不要挤成一大段。
- **关键结论 / 数字加粗**:一句话里最该被记住的那个判断、动作或数字,用 `**…**` 突出。
- **分层不堆砌**:不同层级用「加粗小标题 + 其下列表」分开;同层级内容放一起,别从头到尾一段到底。
- **克制、不花哨**:加粗只给真正的重点(每段一两处足够),不要整段加粗、不滥用符号,**不用彩色字**——靠加粗、层次和留白来分主次,保持统一的黑白灰质感。
- **脚本区分「台词」和「指示」**:写脚本 / 分镜时,如果既有要念的口播台词、又有给拍摄剪辑看的导演指示(「指出后果」「建立判断」「输出金句」这类"这段要干什么"的说明),两者必须一眼分得开 —— **要念的台词正常写**(主体),**导演指示 / 段落意图用引用块 `>` 标成旁注**;别把台词和指示混成同样的列表项,让人分不清哪句要念、哪句只是说明。

目标是让人「重点一眼看清、层次一目了然、读起来不费劲」,而不是把每句话都装饰一遍。
...
2. 只修改当前 `clipmind-sop-core-s06-ondemand-chat-format`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-sop-core-s06-ondemand-chat-format/`。

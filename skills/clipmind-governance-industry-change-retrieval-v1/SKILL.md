---
name: clipmind-governance-industry-change-retrieval-v1
description: "当操盘手明确调用“行业变化可信检索”，并希望只在有可信来源、时间和适用边界时纳入行业变化,无来源不阻断主链路时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
只在有可信来源、时间和适用边界时纳入行业变化,无来源不阻断主链路。
...
```yaml
ref: industry_change_retrieval.v1
key: industry_change_retrieval
version: v1
status: active
name: 行业变化可信检索
description: 只在有可信来源、时间和适用边界时纳入行业变化,无来源不阻断主链路。
input_contract:
  required:
  - industry_query
  optional:
...
- 不得让外部检索失败导致整体失败。
sample_refs:
- strategy_current_ip_fit_v1
- content_plan_source_trace_v1
applicable_modules:
- strategy
missing_input_rules:
- 缺少可信来源时返回 no_verified_source。
surface_constraints:
  display_text: 40
  detail_text: true
...
workbench_context: 行业变化 Skill,负责补充外部变化但不阻断主链路。
core_methodology:
...
- 只接受带来源、时间、适用边界和不确定性的行业变化。
- 无可信来源时返回 no_verified_source,下游继续基于档案和资料运行。
- 把行业变化写成可参考背景,不是客户/IP 的正式事实。
input_focus:
- 当前客户/IP 已确认档案和待确认草稿
...
- 没有时间和来源的行业变化不能进入正式策略。
anti_patterns:
- 不要用预设行业话术替代当前资料。
- 不要把没有来源的判断写成事实。
- 不要把方法论表格原样堆给普通用户。
- 不要把外部趋势当作当前 IP 的证据。
example_application: 短视频平台规则变化可作为发布建议背景,但不能替代客户资料。
surface_guidance:
...
2. 只修改当前 `clipmind-governance-industry-change-retrieval-v1`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-governance-industry-change-retrieval-v1/`。

---
name: clipmind-sop-core-s06-ondemand-structured-output
description: "当操盘手明确调用“结构化输出规范”，并希望要求结果严格服从指定字段、类型和格式，避免多余解释、无效 JSON 与缺失必填项时使用。基于当前IP案例材料工作，必须标注证据缺口、风险和人工确认边界；支持逐轮反馈后调优当前Skill。"
---
...
要求结果严格服从指定字段、类型和格式，避免多余解释、无效 JSON 与缺失必填项。
...
```yaml
skill: S06
prompt_key: core/s06_ondemand_structured_output
module_key: core
output_type: ondemand_fragment
trigger: 结构化产出(JSON / 卡片,供前端渲染或 schema 解析)
change_note: 按需片段 —— 从 v1 floor 下沉的输出格式硬约束(只对结构化产出生效)
```

### 原始执行正文

# 按需 · 结构化输出硬约束(产 JSON / 卡片时追加)

- **严格 JSON**:只输出合法 JSON,不附加任何说明、Markdown 标题、注释或 JSON 之外的文本。
- **单行字符串**:字段值禁止换行符或制表符;要分多项,在同一字符串里用「、」或「;」。
- **禁列表符号**:字段值里禁出现 `-` `*` `•` `1.` 这类列表前缀;真要枚举改用数组型字段。
- **字段长度**:卡片摘要 / 标签 / 状态 ≤ 80 字符;说明 / 理由 ≤ 200;数组型每条 ≤ 40、最多 5 条;脚本主体 / 复盘叙事等长文不设硬上限但必须单行。
- **不暴露内部**:输出里不得出现 `prompt_key`、`schema_name`、`run_id`、`tool_call`、`storage_key`、`input_schema`、`raw_prompt` 等内部字段名或值。
- **不暴露任务面板语**:禁出现「必填输入 / 读取资料 / 预计输出 / 建议写回 / 任务配置」这类内部约定,它们不是用户该看到的卡片内容。
...
2. 只修改当前 `clipmind-sop-core-s06-ondemand-structured-output`，不联动修改其他 Skill。
3. 不改 `references/original.md`；它是初始基线。不得削弱证据要求、人工确认边界和禁止外部动作的护栏。
4. 修改后用同一份材料复跑，并追加测试：材料缺失、越界请求、输出契约三个场景。
5. 把版本、反馈原文、改动摘要、复测结果写入根目录 `tuning-records/clipmind-sop-core-s06-ondemand-structured-output/`。
